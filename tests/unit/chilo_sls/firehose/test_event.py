...
from chilo_sls.firehose.event import Event
from chilo_sls.firehose.record import Record
from chilo_sls.common.records.exception import RecordException

from tests.unit.mocks.firehose import mock_event
from tests.unit.mocks.firehose.mock_data_class import MockFireHoseDataClass


class FirehoseEventTest(unittest.TestCase):
    basic_event = mock_event.get_basic()
    schema_path = 'tests/unit/mocks/firehose/openapi.yml'
    expected_body = {
        'lang': 'en-us',
        'sms': True,
        'email': True,
    ...
    def test_event_validate_record_body_with_schema_file(self):
        event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-firehose-body')
    ...
    def test_event_validate_filters_out_record_body_with_schema_file(self):
        event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-firehose-body-wrong')
        self.assertEqual(len(event.records), 0)

    def test_event_validate_raises_exception_record_body_with_schema_file(self):
        try:
            event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-firehose-body-wrong', raise_body_error=True)
            print(event.records)
