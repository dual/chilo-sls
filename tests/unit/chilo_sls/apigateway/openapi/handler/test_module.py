...
from chilo_sls.apigateway.openapi.handler.module import HandlerModule


class HandlerModuleTest(unittest.TestCase):

    def setUp(self):
        file_path = 'tests/unit/mocks/apigateway/openapi/basic.py'
        import_path = 'tests.unit.mocks.apigateway.openapi.basic'
        spec = importlib.util.spec_from_file_location(import_path, file_path)
        handler_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(handler_module)
        self.module = HandlerModule('tests/unit/mocks/apigateway/openapi', file_path, handler_module, 'post', 'chilo_sls/example')

    def test_file_path(self):
        assert self.module.file_path == 'tests/unit/mocks/apigateway/openapi/basic.py'
    ...
    def test_operation_id(self):
        assert self.module.operation_id == 'PostChiloSlsExampleBasicAcaiGenerated'

    def test_route_path(self):
        assert self.module.route_path == '/chilo_sls/example/basic'
    ...
    def test_tags(self):
        self.assertListEqual(['chilo_sls-example'], self.module.tags)
    ...
    def test_required_path_params(self):
        assert self.module.request_body_schema_name == 'post-chilo-sls-example-basic-request-body'

    def test_response_body_schema_name(self):
        assert self.module.response_body_schema_name == 'post-chilo-sls-example-basic-response-body'
