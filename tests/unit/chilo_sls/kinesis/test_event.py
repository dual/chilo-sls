...
from chilo_sls.kinesis.event import Event
from chilo_sls.kinesis.record import Record
from chilo_sls.common.records.exception import RecordException

from tests.unit.mocks.kinesis import mock_event
from tests.unit.mocks.kinesis.mock_data_class import MockKinesisDataClass


class FirehoseEventTest(unittest.TestCase):
    basic_event = mock_event.get_basic()
    schema_path = 'tests/unit/mocks/kinesis/openapi.yml'
    expected_body = {
        'lang': 'en-us',
        'sms': True,
        'email': True,
    ...
    def test_event_validate_record_body_with_schema_file(self):
        event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-kinesis-body')
    ...
    def test_event_validate_filters_out_record_body_with_schema_file(self):
        event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-kinesis-body-wrong')
        self.assertEqual(len(event.records), 0)

    def test_event_validate_raises_exception_record_body_with_schema_file(self):
        try:
            event = Event(self.basic_event, openapi=self.schema_path, required_body='v1-kinesis-body-wrong', raise_body_error=True)
            print(event.records)
